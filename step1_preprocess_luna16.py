...
def process_pos_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(LUNA16_RAW_SRC_DIR + "CSVFILES/annotations.csv")
...
#    assert abs(sum(direction) - 3) < 0.01
#
#    src_df = pandas.read_csv(settings.LUNA16_RAW_SRC_DIR + "CSVFILES/candidates.csv")
...
def process_auto_candidates_patient(src_path, patient_id, sample_count=100, candidate_type="white"):
    dst_dir = settings.TIANCHI_RAW_SRC_DIR + "extracted_images/" + "_labels/"
    img_dir = settings.TIANCHI_RAW_SRC_DIR + "extracted_images/" + patient_id + "/"
    src_dir = settings.TIANCHI_RAW_SRC_DIR + "csv/val/"
...
def gen_all_mhd_csv():
    ...
    for subject_no in range(10, 15):
        mhd_dir = settings.TIANCHI_RAW_SRC_DIR + "train_subset" + str(subject_no) + "/"
    ...
    df.to_csv(dst_dir + "all_mhd-10-15.csv", index=False)
