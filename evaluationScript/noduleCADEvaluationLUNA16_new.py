...
def computeFROC_bootstrap(FROCGTList,FROCProbList,FPDivisorList,FROCImList,excludeList,numberOfBootstrapSamples=1000, confidence = 0.95):
    ...
    for i in range(numberOfBootstrapSamples):
        print('computing FROC: bootstrap ', i, numberOfBootstrapSamples)
...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1, performBootstrapping=False,numberOfBootstrapSamples=1000,confidence = 0.95):
    '''
...
if __name__ == '__main__':
    ...
    path = '/Users/cyrano/Workspace/DeepLuna/evaluationScript/'
    annotations_filename          = path + 'annotations/annotations.csv'
    annotations_excluded_filename = path + 'annotations/annotations_excluded.csv'
    seriesuids_filename           = path + 'annotations/seriesuids.csv'
    results_filename              = path + 'exampleFiles/submission/all_predictions_world_coord.csv'
    outputDir                     = path + 'exampleFiles/evaluation/'
