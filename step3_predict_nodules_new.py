...
import math
import step2_train_nodule_detector_new
...
def predict_cubes(model_path, continue_job, only_patient_id=None, luna16=False, magnification=1, flip=False, train_data=True, holdout_no=-1, ext_name="", fold_count=2):
    ...
    model = step2_train_nodule_detector_new.get_net(input_shape=(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE, 1), load_weight_path=model_path)
    if not luna16:
        if train_data:
...
#        for magnification in [1, 1.5, 2]: 
        predict_cubes("models/model_final_nodia__fs_best.hd5", CONTINUE_JOB, only_patient_id=only_patient_id, magnification=1, flip=False, train_data=False, holdout_no=None, ext_name="tianchi_test2_fs_final_new")
